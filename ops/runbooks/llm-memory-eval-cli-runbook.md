...
- 汇总报告：
  - [baseline-audit-summary.json](E:/1projects/claude/Vortex/ops/eval-fixtures/baselines/20260603-v3-1-real-agent-workload-official-strict-audit-003/baseline-audit-summary.json:1)
- 结果：
  - `OverallPassed = true`
...
当前 v3 real agent workload 是正式 Agent 工作负载 strict 评测集：

- 数据集：`classpath:llm-memory-eval-set-v3-real-agent-workload.json`
- case 数量：12
...
- 定位：验证更接近长任务 Agent 工作流的记忆行为，包括状态更新、跨任务 blocker、偏好变更、checkpoint 后续执行、branch 决策、工具策略和规划优先级。

v3 工作负载说明和 Phase 3 晋升决策见：
...
`run-llm-memory-baseline-audit.ps1` 会按 `-DatasetLocation` 自动推断 `BaselineProfile` 和 `StrictVerifierProfile`。需要覆盖时可显式传入 `-BaselineProfile` 和 `-StrictVerifierProfile`。
...
v3 real agent workload strict audit 示例：
